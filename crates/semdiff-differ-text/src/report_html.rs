...
impl DetailReporter<TextDiff, FileLeaf, HtmlReport> for TextDiffReporter {
    ...
    fn report_unchanged(&self, _name: &str, _diff: TextDiff, _reporter: &HtmlReport) -> Result<(), Self::Error> {
        todo!()
    }

    fn report_modified(&self, _name: &str, _diff: TextDiff, _reporter: &HtmlReport) -> Result<(), Self::Error> {
        todo!()
    }
